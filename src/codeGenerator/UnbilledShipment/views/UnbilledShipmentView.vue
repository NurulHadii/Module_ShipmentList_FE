...
import { UnbilledShipmentStore } from "@/codeGenerator/order_ocean_v1/stores/UnbilledShipmentStore";
import UnbilledShipmentService from "@/codeGenerator/order_ocean_v1/services/UnbilledShipmentService.ts";
...
const onSaving = async (e) => {
    e.cancel = true;
    if (e.changes.length) {
        e.promise = await UnbilledShipmentService.ProcessBatchRequest("Default", e.changes, e.component);
    }
};

const loadDefaultOptionGrid = async () => {
gridDataTemplate.value = await UnbilledShipmentStore();
